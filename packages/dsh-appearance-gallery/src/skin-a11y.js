...
/** 创建 a11y 注入器。 */
export function createA11yInjector({ a11y, log = console, doc = typeof document === 'undefined' ? undefined : document }) {
  ...
  const inject = (skinId) => {
    ...
    // 幂等：同一皮肤不重复注入。
    let tag = doc.querySelector(`style[data-theme-gallery-a11y="${skinId}"]`)
  ...
  const remove = (skinId) => {
    ...
    for (const el of Array.from(doc.querySelectorAll(`style[data-theme-gallery-a11y="${skinId}"]`))) el.remove()
  }
