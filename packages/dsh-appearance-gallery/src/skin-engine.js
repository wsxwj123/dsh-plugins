...
export function validateCustomBundle(clientText) {
  ...
  while ((m = ctxRe.exec(clientText)) !== null) {
    if (!CTX_WHITELIST.includes(m[1])) badCtx.push(m[1])
  }
  if (badCtx.length > 0) {
...
export function createSkinEngine({
  ...
  return {
    ...
    deactivateSkin() {
      ...
      if (snapshot !== null) restoreSnapshot(snapshot, doc)
      for (const [, h] of handles) {
        for (const attr of h.bodyAttrs) doc.body.removeAttribute(attr)
    ...
    teardownSkins() {
      ...
      for (const el of Array.from(doc.querySelectorAll('style[data-theme-gallery-skin], style[data-theme-gallery-a11y]'))) {
        el.remove()
      }
    },
...
async function defaultExecuteScript(code, doc) {
...
function captureSnapshot(entry, doc) {
  const attrs = {}
  if (entry && entry.bodyAttr && doc.body.hasAttribute(entry.bodyAttr)) {
    attrs[entry.bodyAttr] = doc.body.getAttribute(entry.bodyAttr)
  }
  return { attrs, style: doc.body.getAttribute('style') }
}
