...
### 1.2 预期行为（正常路径）
1. 点击删除 → 该会话**立即从列表隐藏**。
2. 界面出现「撤销条」（每待删项一条），文案含**会话标题** + 剩余秒数（从 5 递减）。
3. 撤销条在**面板/视图切换（切去别处再回来）后仍然存在**且剩余秒数连续（不重置不倒桩）。
4. 倒计时结束（5 秒）：会话保持隐藏，host 侧把会话文件移入回收站；**刷新页面后该会话不再出现**（软删除，见 PLAN §7）。
5. **可连续删除多个会话**（P9 恢复多并发；每个会话一条独立撤销条、独立 5 秒倒计时、独立撤销按钮）——撤销交互上"一次只撤销一条"由撤销条逐条独立按钮天然支持。

### 1.3 撤销路径
- 倒计时内点「撤销」→ 撤销条消失，会话**重新出现在列表原位置**，倒计时 5 秒后**不消失**（未执行真实移动），刷新后仍存在。
- 撤销仅对"尚未到点"的条目有效。
...
- **「删除」（审查 I-5，解决归档删除可达性）**：→ 走同一回收站/撤销流程（5 秒撤销条可用）；host 两步（移文件 + 从 `archivedSessionIds` 移除 id）。撤销后回到归档视图；到点则从归档视图与列表消失。
...
| 点「删除」成功 | 行消失 + 撤销条出现；5 秒内撤销 → 行回归档视图；到点 → 从归档视图与列表消失 |
