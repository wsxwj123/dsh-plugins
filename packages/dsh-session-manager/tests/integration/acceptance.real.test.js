...
test('REAL delete live session (no force) -> ok:true, live store no longer blocks', async (t) => {
  ...
  assertCode(t, res, 200, null)
  t.assert.equal(exists(s.dir), false)
  t.assert.ok(exists(path.join(dctx.trash.root, 'sess-running', SESSION_MARKER)))
})
test('REAL delete live + force:false -> ok:true, force no longer gates', async (t) => {
  ...
  assertCode(t, res, 200, null)
  t.assert.equal(exists(s.dir), false)
  t.assert.ok(exists(path.join(dctx.trash.root, 'sess-force-false', SESSION_MARKER)))
})
test('REAL delete live + force:true -> ok:true, force is a compat no-op', async (t) => {
...
test('REAL delete force non-boolean -> 400 invalid-force', async (t) => {
...
test('REAL delete force:true is idempotent (second call also ok)', async (t) => {
