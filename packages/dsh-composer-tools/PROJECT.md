...
- 04 开发：开始（模型=高性价比档）；T1 脚手架完成（package.json/tsconfig/build/tsdown.config/cordis.patch/data 780 条，依赖 symlink 无物理复制坑）；T0 spike 完成（SPIKE-T0.md：capture keydown/setDraft 可达/data-phase 代码依据确认，无假设证伪；menuOpen 判定修正含 inert 态已回填 INTERFACE §3）；开发代理已派出实现 T2-T12
