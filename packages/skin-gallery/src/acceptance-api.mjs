/**
 * acceptance-api.mjs — skin-gallery 公开接口的 Node 接线（供 tests/acceptance 黑盒驱动）。
 *
 * 场景：验收测试在 Node 环境驱动公开接口函数，无 DOM/无 localStorage、不激活真实 bundle。
 * 用「内存 storage 替身 + 内置 9 皮肤 manifest + 包内校验器」构造真实 createCustomSkinApi。
 * 内置皮肤 manifest 与 packages/skin-gallery 构建时的 buildManifest 对齐（license/author/package）。
 */
import { createCustomSkinApi } from './custom-skin.js'
import { validateCustomBundle } from './skin-engine.js'

/** 内置 9 皮肤（与 skins/<id>/skin.json + buildManifest 对齐）。 */
export const BUILTIN_SKINS = [
  { id: 'qq98', name: 'QQ2008 怀旧版', nameEn: 'QQ2008 Retro', author: 'dsh-web-ui', accent: '#2b7cd9', bodyAttr: 'data-dsh-retro', order: 1, package: '@linxin666/dsh-client-ui-skin-qq98', license: 'BSD-3-Clause' },
