// dsh-turn-scrubber client bundle build — mirrors the official DSH
// client-bundle preset (see dsh-better-sidebar's tsdown.config.ts):
// - externals resolve through the loader module table at runtime
//   (CLIENT_EXTERNALS = the web shell's PLATFORM_MODULES seed list + the
...
const PLUGIN_ID = 'dsh-turn-scrubber'
 ...
 *  Virtual ids are REPO-RELATIVE (never absolute machine paths), so the
 *  built bundle contains no local filesystem information. */
