        ...
        : React.createElement('div', { className: 'skin-gallery-grid' }, ...visible.map((item) => {
            ...
            return React.createElement('div', {
              ...
              React.createElement('div', { className: 'skin-gallery-card-actions' },
                React.createElement('button', {
                  type: 'button', className: 'skin-gallery-action' + (previewState.skinId === item.id ? ' is-current' : ''), disabled: busy,
                  onClick: async () => { setBusy(true); try { await previewSkin(item.id) } finally { setBusy(false) } },
                }, previewState.skinId === item.id ? '试穿中' : '试穿'),
                React.createElement('button', {
                  type: 'button', className: 'skin-gallery-action skin-gallery-action-primary' + (previewState.appliedSkinId === item.id ? ' is-current' : ''), disabled: busy,
                  onClick: async () => { setBusy(true); try { await applySkin(item.id) } finally { setBusy(false) } },
                }, previewState.appliedSkinId === item.id ? '已应用' : '应用'),
