...
## 平台范围（2026-08-17 用户补充）

**macOS + Windows 双平台**。约束：
1. 运行时（浏览器端 client bundle）不得有平台相关路径/字体/滚动行为假设；CSS 滚动性能验收在两平台口径一致。
2. 构建脚本（build.mjs、壳校验、静态门禁）必须跨平台可跑：路径一律 `node:path`（不许硬编码 `/` 拼接）、glob 不依赖 shell 展开、不用 macOS 专属命令（`sips`/`pbcopy` 等；图片重编码工具选跨平台方案）。
3. 文档中的安装/验证命令给出 Windows（PowerShell）等价写法；`node --test` glob 引号跑法注明两平台差异。
4. 测试断言不得含 POSIX-only 路径假设。
